...
static void
exit_if_null (const void *ptr, const int line_num, const char *msg)
{
  if (ptr == NULL)
    {
...
static void
exit_if_violates_bounds (const ssize_t val, const ssize_t bound,
...
static void
exit_if_no_mem_error (int memalign_ret, const char *msg, const int line_num)
{
  if (memalign_ret != 0)
    {
...
static void
exit_if_no_mem_error_malloc (void *ptr, const char *msg, const int line_num)
{
  if (ptr == NULL)
    {
      strerror_r (errno, buf, BUFLEN);
...
static void
exit_if_empty_array (const darray *da, const char *msg)
{
...
void
exit_if_array_access_checks_fail (const darray *da, ssize_t row, ssize_t col)
{
  exit_if_null (da, __LINE__, ERR_MSG_POINTER_TO_DARRAY_IS_NULL);
  exit_if_null (da->elems, __LINE__,
                "null pointer to underlying doubles array");
...
dims
exit_if_matrix_multiply_dims_mismatch (const dims dims_a, const dims dims_b)
{
...
void
exit_if_dims_not_equal (const dims dims_a, const dims dims_b, const char *msg)
{
...
void
exit_if_new_dims_smaller (dims dims_old, dims dims_new, const char *msg,
                          const int line_num)
{
  if (dims_old.n_cols > dims_new.n_cols)
    {
      REPORT_ERROR_AND_EXIT (dims_old.n_cols > dims_new.n_cols, line_num,
                             __FILE__, "%s: %s",
                             "dims_old.n_cols > dims_new.n_cols", msg);
    }
  if (dims_old.n_rows > dims_new.n_rows)
    {
      REPORT_ERROR_AND_EXIT (dims_old.n_rows > dims_new.n_rows, line_num,
                             __FILE__, "%s: %s",
                             "dims_old.n_rows > dims_new.n_rows", msg);
    }
}

void
exit_if_new_dims_greater (dims dims_old, dims dims_new, const char *msg,
                          const int line_num)
{
  if (dims_old.n_cols < dims_new.n_cols)
    {
...
ssize_t
darray_get_size (const darray *da)
{
  exit_if_null (da, __LINE__, ERR_MSG_POINTER_TO_DARRAY_IS_NULL);
...
bool
darray_is_square (const darray *da)
{
  return da->dims.n_rows == da->dims.n_cols;
}

static inline double
darray_view_get_elem_no_checks (const darray_view *dv, ssize_t row_num,
                                ssize_t col_num)
{
...
static inline double *
darray_view_get_elem_ptr_no_checks (const darray_view *dv,
                                    const ssize_t row_num, ssize_t col_num)
{
  return &dv->d->elems[((row_num + dv->row_start) * dv->d->dims.n_cols)
                       + col_num + dv->col_start];
}
...
static inline double
darray_view_set_elem_no_checks (const darray_view *dv, ssize_t row_num,
...
darray *
darray_new_uninitialized (const ssize_t n_rows, const ssize_t n_cols)
{
...
darray *
darray_new_zeroed (ssize_t n_rows, ssize_t n_cols)
{
...
darray *
darray_new_initialized_from_doubles (const ssize_t n_rows,
...
darray_view *
darray_view_new (darray *da, ssize_t row_start, ssize_t row_end,
...
void
darray_view_free (darray_view *dv)
{
...
inline double
darray_get_elem_no_checks (const darray *da, ssize_t row, ssize_t col)
{
...
static inline double *
darray_get_elem_ptr_no_checks (const darray *da, ssize_t row, ssize_t col)
{
  const ssize_t where = (row * da->dims.n_cols + col);
  return da->elems + where;
}
...
bool
darray_equal (darray *da_this, darray *da_that)
{
...
static inline void
darray_set_elem_no_checks (darray *da, ssize_t row, ssize_t col, double value)
{
  const ssize_t where = (row * da->dims.n_cols + col);
...
double
darray_get_item (darray *da, ssize_t row, ssize_t col)
{
...
void
darray_set_item (darray *da, ssize_t row, ssize_t col, double value)
{
  exit_if_array_access_checks_fail (da, row, col);
...
bool
darray_is_empty (darray *da)
{
  return darray_get_size (da) == 0;
}

darray *
darray_copy (darray *da)
{
  darray *da_copy
      = darray_new_uninitialized (da->dims.n_rows, da->dims.n_cols);
...
darray *
darray_copy_into (darray *da, darray *da_copy)
{
  exit_if_null (da, __LINE__, ERR_MSG_POINTER_TO_DARRAY_IS_NULL);
...
double
darray_find_max (const darray *da)
{
...
double
darray_find_min (const darray *da)
{
  exit_if_null (da, __LINE__, "cannot find max of null darray");
...
darray *
darray_add (const darray *da, const darray *db)
{
  ...
  for (; i < (len & ~0x3); i += 4)
    {
      const __m256d ma = IS_ALIGNED (&a[i], 32) ? _mm256_load_pd (&a[i])
                                                : _mm256_loadu_pd (&a[i]);
      const __m256d mb = IS_ALIGNED (&b[i], 32) ? _mm256_load_pd (&b[i])
                                                : _mm256_loadu_pd (&b[i]);
...
darray *
darray_view_add (const darray_view *dva, const darray_view *dvb)
{
...
void
exit_if_darray_view_subadd_checks_fail (const darray_view *dva,
...
void
darray_view_add_into (const darray_view *dva, const darray_view *dvb,
                      const darray_view *dvc)
{
...
darray *
darray_view_sub (const darray_view *dva, const darray_view *dvb)
{
  exit_if_null (dva, __LINE__, "first matrix is null");
  exit_if_null (dvb, __LINE__, "second matrix is null");
  const dims dims_a = (dims){ .n_rows = (dva->row_end - dva->row_start),
...
void
darray_view_sub_into (const darray_view *dva, const darray_view *dvb,
                      const darray_view *dvc)
{
  exit_if_darray_view_subadd_checks_fail (dva, dvb, dvc);
...
dims
darray_view_get_dims (const darray_view *dv)
{
...
void
darray_view_fmadd (const darray_view *x11, const darray_view *y11,
                   ...
                   darray_view *z11)
{
  ...
  exit_if_dims_not_equal (darray_view_get_dims (x11),
                          darray_view_get_dims (x12),
                          "x11 and x12 dims must be equal");
  exit_if_dims_not_equal (darray_view_get_dims (y11),
                          darray_view_get_dims (y21),
  ...
  exit_if_dims_not_equal (dims_z11_expected, darray_view_get_dims (z11),
                          "exp");
...
darray *
darray_sub (darray *da, darray *db)
{
  exit_if_null (da, __LINE__, "first matrix is null");
  exit_if_null (db, __LINE__, "second matrix is null");
...
static inline void
darray_print_spaces (int n_spaces, FILE *out)
{
...
static inline void
darray_print_spaced_dots (int n_chars, FILE *out)
{
...
static inline void
darray_print_part_of_row (const darray *da, const char float_format[],
                          int start, int end, int row, FILE *out)
{
  ssize_t j = start;
  for (; j < end; j++)
    {
...
static inline void
darray_print_truncated_row (const darray *da, const char float_format[],
...
static inline void
darray_print_first_row (darray *da, FILE *out, const char *float_format,
                        int n_chars)
{
  fprintf (out, "darray([[");
  if (da->dims.n_rows > 0)
    {
      if (n_chars * da->dims.n_cols >= MAX_PRINT_WIDTH)
        ...
        {
          darray_print_part_of_row (da, float_format, 0, da->dims.n_cols, 0,
                                    out);
        }
    }
  fprintf (out, "]\n");
...
static inline void
darray_print_rows_from (darray *da, ssize_t start, ssize_t end,
                        const char float_format[], int n_chars, FILE *out)
{
  for (ssize_t i = start; i < end; i++)
    {
      ...
      if (n_chars * da->dims.n_cols >= MAX_PRINT_WIDTH)
        ...
        {
          darray_print_part_of_row (da, float_format, 0, da->dims.n_cols, i,
                                    out);
        }
      fprintf (out, "]\n");
    }
...
static inline void
print_vertical_dots (int n_chars, FILE *out)
{
  const ssize_t n_elems = (MAX_PRINT_WIDTH >> 1) / n_chars;
...
darray_print_mid_rows (darray *da, FILE *out, const char *float_format,
                       int n_chars)
{
  if (da->dims.n_rows > MAX_PRINT_HEIGHT)
    ...
    {
      darray_print_rows_from (da, 1, da->dims.n_rows - 1, float_format,
                              n_chars, out);
      return da->dims.n_rows - 1;
    }
}
static inline void
darray_print_last_row (darray *da, FILE *out, const char *float_format,
                       int n_chars)
{
  fprintf (out, "        [");
  if (n_chars * da->dims.n_cols >= MAX_PRINT_WIDTH)
    {
      darray_print_truncated_row (da, float_format, n_chars,
...
void
cubic (const darray *da, const darray *db, darray *dc)
{
...
void
darray_view_cubic_matmul (const darray_view *dva, const darray_view *dvb,
                          const darray_view *dvc)
{
...
darray *
cache_friendly_transpose (const darray *da)
{
...
darray *
sse_2pack_transpose (const darray *da)
{
  darray *transposed
...
darray *
darray_transpose (const darray *da)
{
  return cache_friendly_transpose (da);
}

static void
darray_pad_rows (darray *da, darray *da_copy)
{
...
static void
darray_strip_rows (darray *da, darray *da_copy)
{
...
static void
darray_strip_cols (darray *da, darray *da_copy)
{
  double *s = da->elems;
  double *t = da_copy->elems;
...
static void
darray_pad_cols (darray *da, darray *da_copy)
{
  double *s = da_copy->elems + da->dims.n_cols;
  for (ssize_t i = 0; i < da->dims.n_rows; i++, s += da_copy->dims.n_cols)
    {
      memset (s, '\0',
              (da_copy->dims.n_cols - da->dims.n_cols) * sizeof (double));
    }
}

darray *
darray_pad (darray *da, dims new_dims)
{
  exit_if_null (da, __LINE__, ERR_MSG_POINTER_TO_DARRAY_IS_NULL);
...
darray *
darray_strip (darray *da, dims new_dims)
{
  exit_if_null (da, __LINE__, ERR_MSG_POINTER_TO_DARRAY_IS_NULL);
...
void
darray_partition (darray *d, darray_view **dv00, darray_view **dv01,
...
darray_view *
darray_view_from_darray (darray *d)
{
  return darray_view_new (d, 0, d->dims.n_rows, 0, d->dims.n_cols);
}

void
darray_view_partition (const darray_view *dv, darray_view **dv00,
                       ...
                       darray_view **dv11)
{
  ...
  *dv00 = darray_view_new (dv->d, row_start, row_start + half_n_rows,
...
darray *
darray_from_view (darray_view *dv)
{
...
void
strassen_impl (darray_view *A, darray_view *B, darray_view *C, int cur_depth,
               int max_depth)
{
...
static inline double
darray_view_get_elem (darray_view *dv, ssize_t row_num, ssize_t col_num)
{
...
static inline double
darray_view_set_elem (darray_view *dv, ssize_t row_num, ssize_t col_num,
                      double elem)
{
  const ssize_t n_rows_view = dv->row_end - dv->row_start;
  const ssize_t n_cols_view = dv->col_end - dv->col_start;
...
void
darray_view_multiply (darray_view *dva, darray_view *dvb, darray_view *dvc)
{
  exit_if_null (dva, __LINE__, "first matrix is null");
  exit_if_null (dvb, __LINE__, "second matrix is null");
  dims dims_c
...
void
vectorized_cubic (const darray_view *dva, const darray_view *dvb,
                  const darray_view *dvc)
{
  // base case
  ...
  darray *dt d_autofree
      = darray_new_zeroed (dims_temp.n_rows, dims_temp.n_cols);
...
darray *
darray_multiply (darray *da, darray *db)
{
  exit_if_null (da, __LINE__, "first matrix is null");
  exit_if_null (db, __LINE__, "second matrix is null");
...
static inline void
darray_print_empty (FILE *out)
{
  fprintf (out, "darray([[]])\n");
}

static inline int
get_format_string (darray *da, char *buf, ssize_t buflen)
{
  const int n_dec = 2;
  const int n_digits
      = get_number_of_digits (trunc (
            MAX (fabs (darray_find_max (da)), fabs (darray_find_min (da)))))
        + n_dec + 2;
...
void
darray_print (darray *da, FILE *out)
{
  exit_if_null (da, __LINE__, ERR_MSG_POINTER_TO_DARRAY_IS_NULL);
...
void
darray_view_print (darray_view *dv, FILE *out)
{
...
void
darray_view_free_wrapper (darray_view **dv)
{
  darray_view_free (*dv);
#ifdef VERBOSE
  fprintf (stderr, GET_SUCCESS_TEXT ("freed"));
